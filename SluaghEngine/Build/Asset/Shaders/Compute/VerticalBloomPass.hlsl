...
void CS_main(int3 groupIndex_3d : SV_GroupID, int threadIndex : SV_GroupIndex)
{
    ...
    if (threadIndex >= KERNEL_RADIUS && threadIndex < GROUP_THREAD_COUNT - KERNEL_RADIUS)
    {
        ...
        for (kernelStep = KERNEL_STEPS; kernelStep > 0; kernelStep--)
        {
            ...
            {
                int offsetRowIndex = pixelIndex_2d.y + pixelOffset;
        ...
        for (kernelStep = 1; kernelStep < 1 + KERNEL_STEPS; kernelStep++)
        {
            ...
            {
                int offsetRowIndex = pixelIndex_2d.y + pixelOffset;
                int kernelOffset = max(0, offsetRowIndex - (RESOLUTION.y - 1)) * -2;
        ...
        float3 modifiedBloomColor = pow(bloomSteps[optimalStep], BLOOM_FADE_EXPONENT);

        modifiedBloomColor = BLOOM_BASE_MULTIPLIER * (
        BLOOM_ADDITIVE_COLOR_STRENGTH_MULTIPLIER * (modifiedBloomColor.r + modifiedBloomColor.g + modifiedBloomColor.b) * modifiedBloomColor
        );


        outTex[pixelIndex_2d] = float4(modifiedBloomColor, 1);
    }
}

...
namespace SE
{
	namespace Core
	{
		...
		public:
			...
			virtual void AddEntity(const Entity& entity, Graphics::RenderJob& job, Graphics::RenderGroup group = Graphics::RenderGroup::RENDER_PASS_0);
			virtual void RemoveEntity(const Entity& entity);
			virtual void UpdateTransform(const Entity& entity, const DirectX::XMFLOAT4X4& transform);
		protected:
			struct BucketAndID
			{
				Utilz::GUID bucket;

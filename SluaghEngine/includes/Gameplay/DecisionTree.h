			...
			**/
			...
			public:
				...
				*/
				...
				{
					StartProfile;
					ProfileReturn(root->returnAnswer(toClassify, this));
				}
				...
				public:
					...
					Answer returnAnswer(std::vector<IFeature*> &toClassify, DecisionTree* owner)
					{
						...
						if (owner->splitingGroups[featureForSplit] != -1)
						{
							if (splitData == -1)
							{
								ProfileReturnConst(answer);
							}
							else
							{
								ProfileReturn(children[toClassify[splitData]->SplitingGroupForSample()]->returnAnswer(toClassify, owner));
							}
						}
						else
						{
							...
							for (int i = 0; i < numberOfChildren; i++)
							{
								...
								{
									ProfileReturn(children[i]->returnAnswer(toClassify, owner));
								}
								else
								{
							...
							if (value < minValue)
								ProfileReturn(children[minChild]->returnAnswer(toClassify, owner));
							if (maxValue < value)
								ProfileReturn(children[maxChild]->returnAnswer(toClassify, owner));
						}
						ProfileReturnConst(static_cast<Answer>(0));
					};
			...
			double DecisionTree<Answer>::Remainder(VectorWrapper* data, int sizeBeforeSplit)
			{
				...
				ProfileReturnConst(minRemainder);
			}
			...
			double DecisionTree<Answer>::InformationGainFromVector(VectorWrapper* data)
			{
				...
				*/
				...
				ProfileReturnConst(maxGainFromSplit);
			}
				...
				std::vector<VectorWrapper*>& returnVector)
			{
				...
				if(median == lowValue || median == highValue)
				{
					if (data->list.size())
					{
						VectorWrapper* soThatThingDoesntBreak = new VectorWrapper;
						soThatThingDoesntBreak->list = data->list;
						returnVector.push_back(soThatThingDoesntBreak);
					}
					ProfileReturnVoid;
				}
			...
			int DecisionTree<Answer>::Gains(VectorWrapper* data, const std::vector<bool>& leftForSplit)
			{
				...
				ProfileReturnConst(attributeToSplitOn);
			}
				...
				int attributeToSplitOn)
			{
				...
				if (data->list.size())
				{
					Answer latestSampleResult = static_cast<Answer>(0);
					for (auto& sample : data->list)
					{
						if (!sample->Compare(latestSampleResult))
						{
				...
				int size = splitingGroups[attributeToSplitOn];

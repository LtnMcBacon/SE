#ifndef SE_GAMEPLAY_RESTARTING_SEQUENCE_H
#define SE_GAMEPLAY_RESTARTING_SEQUENCE_H
...
namespace SE
{
	namespace Gameplay
	{
		...
		*
		**/
		class RestartingSequence : public IComposite
		{
		private:
			RestartingSequence() = delete;
		protected:
		...
		public:
			RestartingSequence(EnemyBlackboard* enemyBlackboard, GameBlackboard* gameBlackboard);
			~RestartingSequence();

			IBehaviour* CopyBehaviour(GameBlackboard* gameBlackboard, EnemyBlackboard* enemyBlackboard) const override
			{
				RestartingSequence* toPass = new RestartingSequence(enemyBlackboard, gameBlackboard);

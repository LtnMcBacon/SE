...
bool SE::Test::RandRoomTest::Run(SE::Utilz::IConsoleBackend* console)
{
	StartProfile;
	...
	auto Door = Utilz::GUID("Door.mesh");
	...
	rH->LoadResource("RoomGeneration.txt", [&nrOfRooms, &RoomArr](auto GUID, auto data, auto size)
	{
	...
	for (int x = 0; x < 25; x++)
	{
		for (int y = 0; y < 25; y++)
		{
			...
			{
				rm.CreateRenderableObject(entities[numberOfEntitesPlaced], Door);
				rm.ToggleRenderableObject(entities[numberOfEntitesPlaced], true);
				tm.SetPosition(entities[numberOfEntitesPlaced], DirectX::XMFLOAT3(x + 0.5f, 0.5f, y + 0.5f));

				tm.SetRotation(entities[numberOfEntitesPlaced], 0.0f, testRoom->FloorCheck(x,y) , 0.0f);
				numberOfEntitesPlaced++;
			}
